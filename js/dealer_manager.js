...
if (!window.DealerManager) {
    window.DealerManager = class DealerManager {
        ...
        async sendWhatsAppMessage(dealerName) {
            ...
            if (messageBody && messageBody.toLowerCase() === 'lazafron') {
                document.getElementById('wa-message-body').value = '';
                if (!phone) { if (window.Toast) window.Toast.error('No phone number found.'); return; }

                const leadPhone = phone.length === 10 ? '91' + phone : phone;
                ...
                try {
                    ...
                    import('./utils/toast.js').then(m => m.Toast.success('Chat history deleted.'));
                } catch (e) {
                    console.error('[lazafron] Failed to delete chat:', e);
                    import('./utils/toast.js').then(m => m.Toast.error('Failed to delete: ' + e.message));
                }
                return;
            }
